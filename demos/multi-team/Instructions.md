...
Before we deploy the gateway, we need to create https certificates. We will create a Certificate Manager. We will create 2 certificates for app-a and app-b

```sh
gcloud certificate-manager certificates create app-a-gtw-cert \
    --domains="app-a.endpoints.${PROJECT_ID}.cloud.goog"
gcloud certificate-manager certificates create app-b-gtw-cert \
    --domains="app-b.endpoints.${PROJECT_ID}.cloud.goog"
```
We could have used wildcard's for app names, but this makes the setup easier.

Will will allow the load balancer to reference the certificate via a certificate map. We will create a certificate map.

```sh
gcloud certificate-manager maps create apps-cert-map
```

Let us create an entry for each of the certificates created in the earlier map.
...
In this example, we use the reference the certificate we create through an annotation(!) that references certmaps.
